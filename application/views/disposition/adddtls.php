  ...
  <div class="content-wrapper">
	...
	<?php $this->load->view('template/message');?>	
      <div class="container-fluid">
        <!-- Info boxes -->
        <form action="<?=base_url('Disposition/saveDispositionDtls')?>" method="post">
         <div class="row">
          <!-- left column -->
          <div class="col-md-12">
        ...
        <table id="example1" class="table table-bordered">
                ...
                <input type="hidden" value="<?=$secdata[0]->sc_id?>" name="a_sec" id="a_sec" />
                </tr>
                <tr>
                  <th>Sl. No</th>				                                      
                  <th>Name</th>				  
                 ...
                 <?=$r->stname?>
                  ...
                  <input type="hidden" id="st_id" name="st_id[]" value="<?=$r->st_id?>" />
                    <select class="form-control select2" style="width: 100%;" id="d_type<?=$r->st_id?>" name="d_type[]" >		
                        ...
                        <?php if(in_array($r->st_id,$duty_assn)) {?>
                          <option value="<?=$dutytype[$i]?>" <?=$dutytype[$i]=='L/O'?'selected':''?>><?=$dutytype[$i]?></option>                                
                        <?php }elseif($staff_leave[$r->st_id]){?>
                          <option value="<?=$dutytype[$i]?>" <?=$dutytype[$i]==$staff_leave[$r->st_id]?'selected':''?>><?=$dutytype[$i]?></option>                                
                        <?php }else{ ?>	
